...
router.get('/patients', requireToken, (req, res, next) => {
  Patients.find()
    ...
    .then(patients => {
      ...
      return patients.map(patients => patients.toObject())
    })
...
router.post('/patients', requireToken, (req, res, next) => {
  ...
  req.body.patients.owner = req.user.id

  Patients.create(req.body.patients)
...
router.patch('/patients/:id', requireToken, removeBlanks, (req, res, next) => {
  ...
  Patients.findById(req.params.id)
    ...
    .then((patient) => res.sendStatus(204).json({ patient: patient.toObject() }))
    // if an error occurs, pass it to the handler
    .catch(next)
})
